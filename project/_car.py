...
img_ori = cv2.imread('LPR_Project/project/image/1.jpg')  # 이미지 불러오기
...
MIN_AREA = 80  # boundingRect(사각형)의 최소 넓이
...
for i, matched_chars in enumerate(matched_result):
    ...
    test = (sorted_chars[-1]['y'] - sorted_chars[0]['y'] + sorted_chars[-1]['h'])
    ...
    cv2.rectangle(img_out, pt1=(info['x'], info['y']), pt2=(
        info['x']+info['w'], info['y']+info['h']), color=(255, 0, 0), thickness=2)
